...
/** Server reads (level word count, preview words) throw by design — this catches that,
 *  distinct from the client-side practice error state inside PracticeSession itself. */
export default function LevelPracticeError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error("[level-practice-error]", error);
  }, [error]);
